...
def exp21_eval(args):
    ...
    path = osp.join(args.modeldiff_root, f"exp21_{args.dataset}_{args.model2}.pt")
    ...
    if not osp.exists(path):
        ...
        for t in phar:
            ...
            # eval modeldiff
            modeldiff = ModelDiff(model1, model2, test_loader=test_loader, device=device, out_root=out_root, seed=args.seed)
    ...
    vis.plot_accuracy_dist_curve(results["plot"], metrics=["ddm"], path=path.replace(".pt", ".pdf"))
