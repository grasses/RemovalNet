...
def exp21_eval(args):
    ...
    path = osp.join(args.deepjudge_root, f"exp21_{args.dataset}_{args.model2}.pt")
    ...
    if not osp.exists(path):
        ...
        for k, v in results["dist"].items():
            ...
            for x, y in zip(results["topk"]["top1"], normalize(v)):
    ...
    metrics = ["LOD", "LAD"]
    vis.plot_accuracy_dist_curve(results["plot"], metrics=metrics, path=path.replace(".pt", ".pdf"))
