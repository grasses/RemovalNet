...
def fbresnet_augmentor(isTrain):
    """
    Augmentor used in fb.resnet.torch, for BGR images input in range [0,255].
    """
    if isTrain:
        augmentors = [
            GoogleNetResize(),
        ...
        augmentors = [
            ...
            #imgaug.Resize(256, cv2.INTER_LINEAR),
...
class ImgAugTVCompose(imgaug.ImageAugmentor):
...
class SeqImageNetLoader(object):
    """
    ...
    def __init__(self, mode, batch_size=256, shuffle=False, num_workers=25, cache=50000,
            collate_fn=default_collate,  remainder=False, cuda=False, transform=None):
        ...
        ds = td.MapDataComponent(ds, lambda x: np.asarray(Image.open(io.BytesIO(x)).convert('RGB')), 0)
    ...
    def __iter__(self):
        for x, y in self.ds.get_data():
            if self.cuda:
                ...
                x = torch.ByteTensor(x).to(sys_args.device)
                y = torch.IntTensor(y).to(sys_args.device)
                # but once they're on the gpu, we'll need them in 
                yield uint8_to_float(x, self.cuda), y.long()
                #yield x, y.long()
            else:
                yield uint8_to_float(torch.ByteTensor(x), self.cuda), torch.IntTensor(y).long()

    def __len__(self):
        return self.ds.size()

def uint8_to_float(x, cuda=False):
    rgb = x.float().div(255)
    if cuda:
        rgb = (rgb - torch.tensor([0.485, 0.456, 0.406]).to(sys_args.device)) / torch.tensor([0.229, 0.224, 0.225]).to(sys_args.device)
    else:
        rgb = (rgb - torch.tensor([0.485, 0.456, 0.406])) / torch.tensor([0.229, 0.224, 0.225])
    rgb = rgb.permute(0,3,1,2) # pytorch is (n,c,h,w)
    return rgb
