...
class CelebA32(VisionDataset):
    ...
    def __init__(
        ...
        root = root.replace("CelebA32", "CelebA2")
        ...
        assert split.lower() in ["val", "train", "test"]
        ...
        self.num_classes = 2
        self.attr_idx = shots
        print(f"-> select: {self.attr_names[shots]} attr_names: {self.attr_names}")
    ...
    def attr_transform(self, label):
        return label[self.attr_idx]
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        for t in self.target_type:
            if t == "attr":
                attr = self.attr_transform(self.attr[index, :])
                target.append(attr)
            elif t == "identity":
