...
class GTSRB(VisionDataset):
    ...
    def __init__(
        ...
        if split == "test":
            self._target_folder = pathlib.Path(osp.join(root, "Final_Test/Images"))
        elif split == "train":
            self._target_folder = pathlib.Path(osp.join(root, f"Training{shots}"))
        self.num_classes = 43
        if download:
            self.download()
        print(f"-> Load GTSRB dataset from:{self._target_folder}")
