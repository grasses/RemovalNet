...
def modify_alexnet(model):
    ...
    def feature_list(self, x):
        ...
        y = self.logits(x)
        out_list.append(y.contiguous().view(y.size(0), -1))
        return y, out_list
...
def alexnet(num_classes=1000, pretrained='imagenet'):
    ...
    model.last_linear = torch.nn.Linear(4096, num_classes, bias=True)
    return model
...
def modify_densenets(model):
    ...
    def feature_list(self, x):
        ...
        out_list.append(features.contiguous().view(features.size(0), -1))
        x = F.relu(features, inplace=True)
        x = F.avg_pool2d(x, kernel_size=7, stride=1)
        x = x.view(x.size(0), -1)
...
def resnet18(num_classes=200, pretrained='imagenet'):
    ...
    model.last_linear = torch.nn.Linear(512, num_classes, bias=True)
    return model

def resnet34(num_classes=200, pretrained='imagenet'):
...
def vgg11_bn(num_classes=200, pretrained='imagenet'):
...
def vgg16_bn(num_classes=200, pretrained='imagenet'):
