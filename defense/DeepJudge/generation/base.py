...
class BaseSeeding:
    def __init__(self, model, arch, test_loader, dataset):
        self.arch = arch
        self.model = model
        ...
        if not osp.exists(self.cache_root):
            os.makedirs(self.cache_root)

    def load_test_samples(self, tag=""):
        fpath = osp.join(self.cache_root, f"test_{self.dataset}-{self.arch}_{tag}.pt")
        if osp.exists(fpath):
